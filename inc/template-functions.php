 ...
 * @return array
 */
function chilevapea_body_classes( $classes ) {
...
add_filter( 'body_class', 'chilevapea_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function chilevapea_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'chilevapea_pingback_header' );
 ...
 * @return array
 */
function chilevapea_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'chilevapea_filter_script_loader_tag', 10, 2 );
 ...
 */
function chilevapea_get_preload_stylesheet_uri( $wp_styles, $handle ) {
 ...
 */
function chilevapea_add_body_style() {
	...
	// Preload content.css.
	$preloads['chilevapea-content'] = chilevapea_get_preload_stylesheet_uri( $wp_styles, 'chilevapea-content' );

	// Preload sidebar.css and widget.css.
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		$preloads['chilevapea-sidebar'] = chilevapea_get_preload_stylesheet_uri( $wp_styles, 'chilevapea-sidebar' );
		$preloads['chilevapea-widgets'] = chilevapea_get_preload_stylesheet_uri( $wp_styles, 'chilevapea-widgets' );
	}

	// Preload comments.css.
	if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
		$preloads['chilevapea-comments'] = chilevapea_get_preload_stylesheet_uri( $wp_styles, 'chilevapea-comments' );
	}

	// Preload front-page.css.
	global $template;
	if ( 'front-page.php' === basename( $template ) ) {
		$preloads['chilevapea-front-page'] = chilevapea_get_preload_stylesheet_uri( $wp_styles, 'chilevapea-front-page' );
	}
...
add_action( 'wp_head', 'chilevapea_add_body_style' );
 ...
 */
function chilevapea_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'chilevapea_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function chilevapea_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'chilevapea_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'chilevapea_add_nav_menu_aria_current', 10, 2 );
