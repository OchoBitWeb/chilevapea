 ...
 */
function chilevapea_using_amp_live_list_comments() {
 ...
 */
function chilevapea_add_amp_live_list_pagination_attribute( $markup ) {
 ...
 */
function chilevapea_index_header() {
	if ( is_home() && ! is_front_page() ) {
		...
		<header class="page-header">
			...
			<?php
				...
				printf( esc_html__( 'Search Results for: %s', 'chilevapea' ), '<span>' . get_search_query() . '</span>' );
			?>
			</h1>
		</header><!-- .page-header -->
		<?php
...
function chilevapea_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'chilevapea' ),
 ...
 */
function chilevapea_posted_by() {
	$byline = sprintf(
		/* translators: %s: post author. */
		esc_html_x( 'by %s', 'post author', 'chilevapea' ),
 ...
 */
function chilevapea_post_categories() {
	// Only show categories on post types that have categories.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'chilevapea' ) );
		if ( $categories_list ) {
			/* translators: 1: list of categories. */
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'chilevapea' ) . ' </span>', $categories_list ); // WPCS: XSS OK.
		}
	}
}
 ...
 */
function chilevapea_post_tags() {
	// Only show tags on post types that have categories.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'chilevapea' ) );
		if ( $tags_list ) {
			/* translators: 1: list of tags. */
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'chilevapea' ) . ' </span>', $tags_list ); // WPCS: XSS OK.
		}
	}
}
 ...
 */
function chilevapea_comments_link() {
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					...
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'chilevapea' ),
					array(
						'span' => array(
							'class' => array(),
						),
 ...
 */
function chilevapea_edit_post_link() {
	edit_post_link(
		sprintf(
			wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers */
				__( 'Edit <span class="screen-reader-text">%s</span>', 'chilevapea' ),
				array(
					'span' => array(
						'class' => array(),
					),
 ...
 */
function chilevapea_post_thumbnail() {
 ...
 */
function chilevapea_attachment_in( $post ) {
	if ( ! empty( $post->post_parent ) ) :
		$postlink = sprintf(
			/* translators: %s: original post where attachment was added. */
			esc_html_x( 'in %s', 'original post', 'chilevapea' ),
 ...
 */
function chilevapea_the_attachment_navigation() {
	?>
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php echo esc_html__( 'Post navigation', 'chilevapea' ); ?></h2>
		<div class="nav-links">
			<div class="nav-previous">
				<div class="post-navigation-sub">
					<?php echo esc_html__( 'Previous attachment:', 'chilevapea' ); ?>
				</div>
				<?php previous_image_link( false ); ?>
			</div><!-- .nav-previous -->
			<div class="nav-next">
				<div class="post-navigation-sub">
					<?php echo esc_html__( 'Next attachment:', 'chilevapea' ); ?>
				</div>
